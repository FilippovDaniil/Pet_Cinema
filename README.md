...
- **JDK 17** — `java -version` должен показывать 17
- **Docker Desktop** — запущен
- **4+ GB RAM** для Docker (Kafka + несколько JVM требуют памяти)

### Шаг 1: Сборка Java сервисов

```bash
# В корне проекта Pet_Cinema
./gradlew build -x test
# -x test пропускает тесты для быстроты
# Это создаст .jar файлы в каждом модуле build/libs/
```

Windows (если нет gradlew):
```
...
`--build` пересобирает Docker образы. При первом запуске скачает все базовые образы (~2-3 GB).

### Шаг 3: Дождаться старта

Порядок старта (занимает ~2-3 минуты):
1. PostgreSQL × 6, Redis (10-15 сек)
2. Zookeeper + Kafka (20-30 сек)
3. Eureka (30 сек)
4. Все микросервисы (30-60 сек — JVM startup + liquibase/ddl)
5. Frontend (сборка React + Nginx)

Проверка готовности:
```bash
# Eureka dashboard — должны быть видны все сервисы
